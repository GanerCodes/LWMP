...
/* calibration.js */
/* AUTOGENERATED */
 const recalb_t_temp_hack = (𝔡) => 𝔡.recalb_t();

 RGB_calibrate = async (𝔡) => {
   await recalb_t_temp_hack(𝔡);
   let C = [..."RGB"];
   await 𝔡.bulk([`config`, {
       RGB_ORDER: `RGB`
     }],
   ...
   return [C, async (α) => {
     C = C.þF0232((x, ...𝔸) => x != α);
     await 𝔡.scene(`_singlecolor1`, false, -1);
     return [C, async (β) => {
 ...
 prompt_rgb_calibrate = async (𝔡, cb) => {
   let α, β;
   const þ0E27F = popup({
       þ0F2D4: true,
       þF0159: true
   ...
   Ғ(async (...𝔸) => await RGB_calibrate(𝔡));
 }

 led_calibrate = þF01EE((𝘴, 𝘳) => async (𝔡, cb) => {
   await recalb_t_temp_hack(𝔡);
 ...
 prompt_led_calibrate = async (𝔡, cb) => {
   let α, β;
   const þ0E27F = popup({
       þ0F2D4: true,
       þF0159: true
   ...
   load();
   const [𝘴, 𝘳] = led_calibrate(𝔡, (x, ...𝔸) => (þ0E27F.close(), cb(x)));
   while (true) {
     let v = await 𝘳();
     if (!(v && v.length)) break;
     const [n, m, C] = v;
...
/* editorPreview.js */
/* AUTOGENERATED */
 const valid_ungroup_þ0E749 = `.𝘌modes[dragitem]:not(:has(>:only-child))`;
 ...
 const əresizer = (Ѧ, 𝘔þ0E27F) => {
   ...
   const þ0E27F = mkə(false, [
     [`𝐶`, `resizer`]
   ], [((lþ0E27F = mkə(`T`, false, þF0832Util.len(i2m(Ѧ, 𝘔þ0E27F.idx)))))]);
   þ0E27F.dragstart = (𝜺) => {
     const y0 = 𝜺.Sy;
     const stot0 = Ѧ.stot;
     const M = i2m(Ѧ, 𝘔þ0E27F.idx);
...
/* AUTOGENERATED */
 ...
 prompt_editor = (cb, 𝔱, 𝐬) => {
   if (test_dat) 𝐬.þF0832 = test_dat;
   const Ѧ = {
     𝔱,
     𝐬,
     þF0832: 𝐬.þF0832,
