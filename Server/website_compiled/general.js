/* AUTOGENERATED */
 const  𝑜Replace  =   (x,y,...𝔸)=> { y  =  structuredClone(y) ; 
                  for (k in x)  delete  x[k] ; 
                  return 𝑜 . assign(x , y) ;  } ; 
𝗉  =   (...𝔸)=> {  let  r , b , p  =   new  Promise( (...𝔸)=> [r , b] = 𝔸) ; 
        return p . r = r , p . b = b , p ;  }
𝘔  =   (...𝔸)=> { 
   const  M  =  [𝗉()] ; 
   const  𝘴  =   (...𝔸)=> (M . at(-1) . r(𝔸) , M . push(𝗉())) ; 
   const  𝘳  =   async  (...𝔸)=> ( (x,...𝔸)=> (M . shift() , x))( await  M[0])
   return [𝘴 , 𝘳] ;  }
þF01EE  =  (ƒ) =>  (...𝔸)=> { 
   const  [[𝚂𝘴 , 𝙲𝘳] , [𝙲𝘴 , 𝚂𝘳]]  =  [𝘔() , 𝘔()]
  ƒ(𝚂𝘴 , 𝚂𝘳)( ... 𝔸) ; 
   return [𝙲𝘴 , 𝙲𝘳] ;  }

fit_size  =  (𝐭 , w , sz = 100 , st =  ""  , Δ = 5) => {
   while (measureText(𝐭 , sz ,  "" ) . width >= w) sz  -=  Δ ; 
   return `font-size:${sz}px;` ;  }

𝘋𝘉  =  𝔏𝔖 . þ02193("tokens" , {}) ; 
updateDB  =   ()=> 𝔏𝔖 . þ02191("tokens" , 𝘋𝘉) ; 

add_check_tok  =  (𝐭) => {
   if ( ! (𝐭 in 𝘋𝘉)) { 𝘋𝘉[𝐭] = { scenes : {} ,  devs : {} } ; 
               updateDB() ;  }
   return 𝘋𝘉[𝐭] ;  }
del_tok  =  (𝐭) => {  print (`Deleting token "${𝐭}"`) ; 
                   delete  𝘋𝘉[𝐭] ; 
                  updateDB() ;  }
get_toks      =   (...𝔸)=> 𝒪k(𝘋𝘉)
get_devs      =  (𝐭) => add_check_tok(𝐭) . devs
get_scenes    =  (𝐭) => add_check_tok(𝐭) . scenes
copy_scene  =  (𝐭 , o , n) => {
   const  𝐒  =  add_check_tok(𝐭) . scenes ; 
   if ( ! (o in 𝐒)  ||  (n in 𝐒))  return  false  ; 
  𝐒[n]  =  𝐒[o] ; 
  updateDB() ; 
   return  true  ;  }
move_scene  =  (𝐭 , o , n =  undefined ) => { 
   const  𝐒  =  add_check_tok(𝐭) . scenes ; 
   if ( ! (o in 𝐒))  return  false  ; 
   if (n !==  undefined )  if ( ! copy_scene(𝐭 , o , n))  return  false  ; 
   delete  𝐒[o] ; 
  updateDB() ; 
   return  true  ;  }
add_scene  =  (𝐭 , 𝐬 , override) => { 
  name  =  𝐬 . name ;  𝐬  =  { ... 𝐬} ;   delete  𝐬 . name ; 
   const  𝐒  =  add_check_tok(𝐭) . scenes ; 
   if ( ! override  &&  name in 𝐒)  return  false 
  𝐒[name]  =  𝐬 ; 
  updateDB() ; 
   return 𝐬 ;  }

load_devices  =   async  (𝐭) => {
   const  𝐋  =  add_check_tok(𝐭)
   let  r ; 
  
  try{ r  =   await  𝐀(𝐭) . get_devs() ;  }catch(e){}
   if ( ! r  ||   ! r[0]  ||  r . status != 200)  return 𝖡(𝐋 . devs) ; 
  𝐋 . devs  =  {  ... ᴍv(𝐋 . devs ,  (x,...𝔸)=> ({ ... x , offline :  false })) ,   ... r[1] }
  updateDB() ; 
   return 𝖦(𝐋 . devs) ;  }