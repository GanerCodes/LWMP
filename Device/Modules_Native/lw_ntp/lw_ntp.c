...
static mp_obj_t lw_ntp_micros(void) { return mp_obj_new_int_from_ll(esp_timer_get_time()); }
static MP_DEFINE_CONST_FUN_OBJ_0(lw_ntp_micros_obj, lw_ntp_micros);
...
static mp_obj_t lw_ntp_ntp_raw(size_t n_args, const mp_obj_t *args) { // AI
  ...
  if(timeout != -1) { struct timeval tv;
                      tv.tv_sec = timeout;
                      tv.tv_usec = 0;
  ...
  if(T0<0 || T3<0) { T0=T3=-1; }
  
  MP_THREAD_GIL_ENTER();
  return mp_obj_new_tuple(3,(mp_obj_t[]){ mp_obj_new_int_from_ll(T0),
